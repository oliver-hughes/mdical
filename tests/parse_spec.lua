...
describe("parse.line: diagnostics", function()
  ...
  it("will not promote off an inactive keyword timestamp", function()
    local p = parse.line("- [ ] a task DEADLINE: [2026-09-01 Tue]")
    local d = diag(p, "inactive-keyword-timestamp")
    truthy(d, "reported")
    eq(p.kind, nil, "`[...]` is not an entry, keyword or no keyword")
    eq(p.due, nil, "no due date")
    eq(d.fixit.text, "<2026-09-01 Tue>", "fixit makes it active")
    truthy(diag(parse.line("- [ ] a task SCHEDULED: [2026-09-01 Tue]"), "inactive-keyword-timestamp"), "SCHEDULED:")
  end)

  it("reads an active CLOSED: but says org writes it inactive", function()
    local p = parse.line("- [x] done <2026-08-01 Sat> CLOSED: <2026-09-01 Tue>")
    local d = diag(p, "active-closed")
    truthy(d, "reported")
    eq(date.iso(p.closed.date), "2026-09-01", "still read - `.+` needs the completion date")
    eq(d.fixit.text, "[2026-09-01 Tue]", "fixit makes it inactive")
    eq(#parse.line("- [x] done <2026-08-01 Sat> CLOSED: [2026-09-01 Tue]").diagnostics, 0, "quiet when inactive")
  end)
